...
export default async function BlogDetailsPage({ params }: Props) {
  ...
  const [latest, popular, sameCategory] = await Promise.all([
    listLatestBlogs(blog.id, 8),
    listPopularBlogs(blog.id, 6),
    listBlogsByCategory(blog.category, blog.id, 6),
  ]);

  const bottomRelated =
    sameCategory.length > 0
      ? sameCategory
      : latest.filter((p) => !popular.some((pop) => pop.id === p.id)).slice(0, 6);
        ...
        <div className="container mx-auto px-8 lg:px-6 xl:px-16">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-12">
            ...
            <div className="lg:col-span-4">
              <BlogSidebar latest={latest} popular={popular} isAr={isAr} />
            </div>
          </div>
          ...
          {bottomRelated.length > 0 ? (
            <section className="mt-16 border-t border-black/10 pt-16">
              ...
              <div className="mt-10">
                <BlogList posts={bottomRelated} isAr={isAr} readmore={readmore} />
              </div>
            </section>
          ) : null}
        </div>
