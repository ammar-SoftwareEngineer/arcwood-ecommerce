import ButtonMore from "@/components/ui/ButtonMore";
import HeaderSection from "@/components/ui/HeaderSection";
import WhyFeatures from "@/components/why/WhyFeatures";
import WhyOverlappingImages from "@/components/why/WhyOverlappingImages";
import { getWhyUs } from "@/lib/api/why";
import { getLocale } from "next-intl/server";



export default async function Why() {
  const data = await getWhyUs();

  if (!data) return null;

  const isAr = (await getLocale()) === "ar";

  return (
    <section className="why-section">
      <div className="container mx-auto px-8 lg:px-6 xl:px-16">
        <div className="grid grid-cols-12 gap-8 lg:items-center xl:gap-12">
          <div className="col-span-12 lg:col-span-7 xl:col-span-6">
            <WhyOverlappingImages
              primarySrc={data.primary_image || ""}
              secondarySrc={data.secondary_image || ""}
              primaryAlt={(isAr ? data.primary_image_alt_ar : data.primary_image_alt) ?? ""}
              secondaryAlt={(isAr ? data.secondary_image_alt_ar : data.secondary_image_alt) ?? ""}
            />
          </div>

          <div className="col-span-12 flex flex-col gap-2 lg:col-span-5 xl:col-span-6">
            <HeaderSection
              className="about-header"
              subtitle={(isAr ? data.subtitle_ar : data.subtitle) ?? ""}
              title={(isAr ? data.title_ar : data.title) ?? ""}
            />
            <p className="mt-6 mb-5 text-lg text-neutral-600">
              {isAr ? data.description_ar : data.description}
            </p>
            <WhyFeatures features={data.features} isAr={isAr} />
            <div className="flex justify-start pt-2 [&_button]:mx-0 [&_button]:mt-4">
              <ButtonMore
                href={data.cta_href ?? "/about"}
                text={(isAr ? data.cta_label_ar : data.cta_label) ?? ""}
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
